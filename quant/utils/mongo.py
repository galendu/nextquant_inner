...
def initMongodb(host="127.0.0.1", port=27017, username="", password="", dbname="admin"):
    ...
    logger.info("create mongodb %s connection pool." % dbname)
    return MONGO_CONN
...
class MongoDBBase(object):
    ...
    def __init__(self, db, collection, host=None, port=None, username=None, password=None):
        """ 初始化
        @param db 数据库
        @param collection 表
        """
        ...
        self._conn = initMongodb(host=host, port=port, username=username, password=password, dbname=self._db)
        self._cursor = self._conn[self._db][collection]
