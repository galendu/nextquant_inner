...
class MyLogData:
    ...
    def __init__(self, collection, host=None, port=None, username=None, password=None):
       ...
       self._db = MongoDBBase(self._db, collection, host=host, port=port, username=username, password=password)

    async def save_order_info(self, t, info):
        data = {
            't': t,
            'info': info
        }
        object_id = await self._db.insert(data)


class MyAssetData:
    def __init__(self, collection, host=None, port=None, username=None, password=None):
       self._db = 'robot_asset'
       self._collection = collection
       self._db = MongoDBBase(self._db, self._collection, host=host, port=port, username=username, password=password)

    async def save_asset_info(self, t, amount):
        data = {
            't': t,
            'amount': amount
        }
...
class MyTableData:
    ...
    async def save_table_info(self, info):
        data = {
            'title': info['title'],
            'columns': info['columns'],
            'rows': info['rows']
        }
        table_id = await self._db.insert(data)
        return table_id

    async def update_table(self, info):
        spec = {
            'title': info['title'],
            'columns': info['columns'],
        }

        update_fields = {"$set": {"rows": info['rows']}}
        count = await self._db.update(spec, update_fields=update_fields, upsert=True)
        return count
