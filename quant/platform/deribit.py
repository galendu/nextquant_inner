...
class DeribitTrade(Websocket):
    ...
    async def revoke_order(self, *order_nos):
        ...
        * NOTE: 单次调换最多只能撤销100个订单，如果订单超过100个，请多次调用
        ...
        if len(order_nos) == 1:
            ...
            if error:
                logger.info(error)
                if error.get('code') == 11044 or error.get('code') == 10004:  # 订单不是未成交状态或不存在
                    success, error = await self.get_order_status(order_nos[0])
                    if error:
                        logger.info('查询订单详情失败：', error)
                        if error.get('code') == 10004:
                            await self._special_update_order(order_nos[0])
                    else:
                        logger.info('查询订单详情成功:', success)
                return None, error
            else:
        ...
        if len(order_nos) > 1:
            ...
            for order_no in order_nos:
                ...
                if e:
                    logger.info(error)
                    if e.get('code') == 11044 or e.get('code') == 10004:  # 订单重复撤销
                        success, error = await self.get_order_status(order_no)
                        if error:
                            logger.info('查询订单详情失败：', error)
                            if error.get('code') == 10004:
                                await self._special_update_order(order_no)
                        else:
                            logger.info('查询订单详情成功:', success)
    ...
    async def get_order_status(self, order_no):
        ...
        if error:
            ...
            order = self._update_order(success)
            if self._order_update_callback:
                SingleTask.run(self._order_update_callback, copy.copy(order))
        return success, error
