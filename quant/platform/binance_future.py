...
class BinanceFutureRestAPI:
    ...
    async def create_order(self, action, symbol, price, quantity, client_order_id=None, order_type=ORDER_TYPE_LIMIT):
        """ Create an order.
        Args:
...
class BinanceFutureTrade:
    ...
    async def create_order(self, action, price, quantity, order_type=ORDER_TYPE_LIMIT):
        ...
        result, error = await self._rest_api.create_order(action, self._raw_symbol, price, quantity, client_order_id, order_type=order_type)
        if error:
            return None, error
    ...
    async def revoke_order(self, *order_nos):
        ...
        if len(order_nos) == 0:
            ...
            for order_info in order_infos:
                ...
                if error:
                    await self.get_order_status(self._raw_symbol, order_info["orderId"], order_info["clientOrderId"])
                    return False, error
            return True, None
        ...
        if len(order_nos) == 1:
            ...
            if error:
                await self.get_order_status(self._raw_symbol, order_id, client_order_id)
    ...
    async def get_order_status(self, symbol, order_id, client_order_id):
        order_info, error = self._rest_api.get_order_status(symbol, order_id, client_order_id)
        if not error:
            ...
            if not order:
                ...
                order = Order(**info)
                self._orders[order_no] = order
            order.remain = float(order_info["origQty"]) - float(order_info["executedQty"])
