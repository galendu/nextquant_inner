...
from quant.const import FCOIN_MARGIN
...
__all__ = ("FCoinMarginRestAPI", "FCoinMarginTrade")


class FCoinMarginRestAPI:
    """
    ...
    async def get_margin_account(self, instrument_id):
        ...
        """
        uri = 'broker/leveraged_accounts/account'
        success, error = await self.request('GET', uri, params={'account_type': instrument_id}, auth=True)
        return success, error

    async def borrow(self, instrument_id, currency, amount=0, loan_type='normal'):
        """

        :param instrument_id: 杠杆币对名称
        :param currency: 借款币种名称
        :param amount: 借款数量
        :param loan_type: 借款类型， normal 正常借款（指定借款数量）; all 全部（把剩余资产全部用来借款）
        """

        body = {
            "account_type": instrument_id,
            "currency": currency,
            "loan_type": loan_type
        }
        if loan_type == 'normal':
        ...
        uri = 'broker/leveraged/loans'
        success, error = await self.request('POST', uri, body=body, auth=True)
        return success, error

    async def repayment(self, loan_bill_id):
        """

        :param loan_bill_id:借款单编号
        """
        uri = 'broker/leveraged/repayments/{loan_bill_id}'.format(loan_bill_id=loan_bill_id)
        success, error = await self.request("POST", uri, auth=True)
        return success, error

    async def create_order(self, action, instrument_id, price, quantity,order_type=ORDER_TYPE_LIMIT, account_type=None, exchange='main'):
        """ 下单
    ...
    async def get_candle(self, resolution, instrument_id, before=None, limit=20):
        ...
        """
        uri = 'market/candles/{resolution}/{symbol}'.format(resolution=resolution, symbol=instrument_id)
        params = {
            'limit': limit
        }
        if before:
...
class FCoinMarginTrade:
    ...
    def __init__(self, **kwargs):
        ...
        # 初始化 REST API 对象
        self._rest_api = FCoinMarginRestAPI(self._host, self._access_key, self._secret_key)
    ...
    async def create_order(self, action, price, quantity, order_type=ORDER_TYPE_LIMIT, account_type='margin', exchange='main'):
        """ 创建订单
        ...
        result, error = await self._rest_api.create_order(action, self._raw_symbol, price, quantity, order_type,account_type,exchange)
        if error:
            return None, error
        if result['status'] != 0:
            return None, result
    ...
    async def get_history_kline(self, perid, size):

        success, error = await self._rest_api.get_candle(perid, self._symbol)
        return success, error

    async def get_volatility(self, perid):
        ...
        kline = success['data'][min(success['data'], 5)]
        df = pd.DataFrame(success['data'][:kline])
        df['volatility'] = (df['high'] - df['low']) / df['close']
        return df['volatility'].mean()

# if __name__ == '__main__':
    
#     client = FCoinMarginRestAPI(host='https://api.fcoin.com/v2/', access_key='', secret_key='')

#     from quant.quant import quant
#     quant.initialize()

#     SingleTask.run(client.get_candle, 'M1', 'btcusdt')
